...
function createChatRooms(router) {
    ...
    const buttons = []; // 각 버튼을 담아둡니다
    for (const nft of nfts) {
        const card = el('ion-card', {
            style: { textAlign: 'center', padding: '12px' }
        });
        const header = el('ion-card-header', el('ion-card-title', {
            style: {
                fontSize: '18px',
                fontWeight: '600',
                margin: '0',
    ...
    fetchNftOwnershipStats(address).then(result => {
        nfts.forEach((nft, idx) => {
            ...
            const holderInfo = btn.parentElement?.querySelector('div');
            if (holderInfo)
                holderInfo.textContent = `홀더 ${totalHolders}명`;
