...
class ProfileService extends EventTarget {
     ...
     * 강제로 프로필을 주입합니다.
     */
    setProfile(account, nickname, bio) {
        const addr = getAddress(account);
    ...
    async #fetchBatch(addresses, force = false) {
        const targets = addresses.filter(a => {
            if (this.#inflight.has(a))
                return false;
            if (!force && !this.#needsRefresh(a))
                return false;
            return true;
        });
        if (!targets.length)
        ...
        try {
            const results = await Promise.all(targets.map(async (addr) => {
                const profile = await fetchProfile(addr);
                return [getAddress(addr), profile];
            }));
    ...
    #emitIfChanged(addr, prev, next) {
        if (prev?.nickname !== next?.nickname ||
            prev?.bio !== next?.bio) {
            this.dispatchEvent(new CustomEvent('profilechange', { detail: { account: addr, profile: next } }));
