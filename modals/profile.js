...
function createProfileModal(router) {
    ...
    const profileCard = el('ion-card', el('ion-card-header', {
        style: `
      text-align: center;
      display: flex;
      ...
      gap: 4px;
    `
    }, el('ion-avatar', { style: 'width:64px;height:64px;margin:auto' }, avatar), nicknameSpan, addressSpan, bioSpan));
    // 최초 로드 시 캐시에서 불러옴
    ...
    const menuItem = (icon, title, subtitle = '', onClick) => el('ion-item', { button: !!onClick, onclick: onClick }, el('ion-icon', { name: icon, slot: 'start' }), el('ion-label', el('h2', title), subtitle ? el('p', subtitle) : undefined));
    const modalContent = el('ion-content.ion-padding', profileCard, el('ion-list', menuItem('person-circle', '프로필 편집', '닉네임과 자기소개를 수정합니다.', () => {
    ...
    const modalHeader = el('ion-header', el('ion-toolbar', el('ion-title', '프로필'), el('ion-buttons', { slot: 'end' }, el('ion-button', { onclick: () => modal.dismiss() }, '닫기'))));
